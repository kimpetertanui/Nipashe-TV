<?php
	...
	// get image file from table
	$sql_query = "SELECT channel_image FROM tbl_channel WHERE category_id = ?";
	...
	if ($stmt_menu->prepare($sql_query)) {
		...
		$stmt_menu->bind_result($image_data['channel_image']);
	}

	// delete all menu image files from directory
	while ($stmt_menu->fetch()) {
		$channel_image = $image_data[channel_image];
		$delete_image = unlink('upload/'."$channel_image");
	}

	$stmt_menu->close();

	// delete data from menu table
	$sql_query = "DELETE FROM tbl_channel WHERE category_id = ?";
